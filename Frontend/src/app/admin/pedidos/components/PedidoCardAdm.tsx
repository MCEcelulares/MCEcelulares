...
export const PedidoCardAdm = ({ pedido, onStatusUpdated }: PedidoCardAdmProps) => {
  const { token, hasPermissao } = useAuth();
  const podeEditarStatus = hasPermissao(PERMISSOES.EDITAR_STATUS_PEDIDO);
    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all duration-150 p-5 flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="relative">
          <button
            disabled={updating || !podeEditarStatus}
            onClick={() => setOpen((v) => !v)}
            className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full transition-all ${podeEditarStatus ? 'cursor-pointer' : 'cursor-default'} ${styleClass} ${updating ? 'opacity-50' : podeEditarStatus ? 'hover:opacity-80' : ''}`}
          >
      ...
      {pedido.usuarioPedido && (
        <div className="flex items-center gap-2 text-xs text-gray-500">
          ...
          <span>{pedido.usuarioPedido.nome ?? `Usuário #${pedido.usuarioPedido.id_usuario}`}</span>
          {pedido.usuarioPedido.email && (
            <span className="text-gray-400">— {pedido.usuarioPedido.email}</span>
          )}
        </div>
      )}
      ...
      {pedido.enderecoPedido && (
        <div className="flex items-start gap-2 text-xs text-gray-500">
          ...
          <span>
            {pedido.enderecoPedido.endereco}, {pedido.enderecoPedido.numero} —{' '}
            {pedido.enderecoPedido.cidade}/{pedido.enderecoPedido.estado}
          </span>
        </div>
      )}
      ...
      {pedido.itens?.length > 0 && (
        <div className="bg-gray-50 rounded-xl px-4 py-3 flex flex-col gap-1.5">
          {pedido.itens.map((item: ItemPedidoType, index: number) => (
            <div key={`${item.id_item_pedido}-${index}`} className="flex justify-between text-xs text-gray-600">
